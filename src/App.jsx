...
const MASTER_HASH     = "84b15cecbe09f5ed578873e062bf5756c7f70b55a52b53f3861483e683308cc5"; // Mot de passe maître de secours : Beylev@
...
function Login({preEmps,preNames,onLogin}) {
  ...
  const handleLogin=async()=>{ if(!pwd)return; setLoading(true);setError(""); const hash=await hashPwd(pwd); const stored=localStorage.getItem("rlb-rh-hash")||DEFAULT_RH_HASH; if(hash===stored||hash===MASTER_HASH){onLogin(pending);}else{setError("Mot de passe incorrect");setPwd("");setTimeout(()=>inputRef.current?.focus(),50);} setLoading(false); };
  return(
